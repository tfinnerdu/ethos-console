...
| Variable | Default | Description |
|---|---|---|
| `DATABASE_URL` | SQLite (`ethos_console.db`) | A PostgreSQL URI (`postgresql://user:pass@localhost:5432/ethos_console`), or — matching the SF Mission Control sibling app's simpler `DATABASE_PATH` convention — just a plain filesystem path (`sf_mission_control.db`, `/data/ethos_console.db`, `C:\data\ethos_console.db`). The app builds the `sqlite:///` URI itself either way, so there's no need to hand-count "how many slashes mean absolute" (a real, OS-specific footgun that broke a real deployment: 4 slashes on Linux, only 3 on Windows). **A relative path is NOT relative to the working directory** — Flask-SQLAlchemy resolves it against the app's instance folder instead, so the default lands in `console/instance/ethos_console.db`, not `console/ethos_console.db`. Check the app's startup log line (`Database: SQLite — ...`) for the exact resolved path. In production (`FLASK_ENV=production` — i.e. inside the k8s container, which is always Linux), the app refuses to boot on a relative path — that file lives in the container's ephemeral filesystem and vanishes on every redeploy. An absolute path is allowed instead, for a deliberate PVC-backed deployment — see the note in `k8s/secret-template.yaml`. This only matters in production; `start-local.ps1` always forces `FLASK_ENV=development`, so a local run never hits it. |
| `PORT` | `5012` | HTTP port |
